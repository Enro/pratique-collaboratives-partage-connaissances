...
--
#### 6. Ajouter un tag sur ces deux notes en cliquant sur `r icons::fontawesome("ellipsis-h")` `Add file property`. 
...
1. Aller dans `r icons::fontawesome("cog")` `Paramètres` > `Modules principaux` > activer le module `Modèles` `r icons::fontawesome("toggle-on")` > cliquer sur `r icons::fontawesome("cog")` pour accéder aux options > définir l'emplacement du dossier modèles comme `Modèles`.
...
Exercices corrigés : ouvrir /Users/antoine/Documents/Enseignement/BUT Infonum/Cours "Pratiques collaboratives et partage de connaissances"/Coffre Obsidian/Exercices corrigés Dataview.md dans Obsidian
